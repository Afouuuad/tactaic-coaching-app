...
import { BASE_ASSET_URL, AUTH_API_END_POINT } from '@/utils/constant';
...
const PlayerLeaderboard = ({ players, navigate }) => {
  ...
  const getFilteredAndSorted = (statKey) => {
    return safePlayers
      .filter(p => {
        const val = statKey === 'contributions' ? p.stats.goals + p.stats.assists : p.stats[statKey];
        return val > 0; // Strictly filter out 0 stats
      })
      .sort((a, b) => {
        const valA = statKey === 'contributions' ? a.stats.goals + a.stats.assists : a.stats[statKey];
        const valB = statKey === 'contributions' ? b.stats.goals + b.stats.assists : b.stats[statKey];
        return valB - valA;
      })
      .slice(0, 3);
  };

  const topGoals = getFilteredAndSorted('goals');
  const topAssists = getFilteredAndSorted('assists');
  const topContributions = getFilteredAndSorted('contributions');
    ...
    <div className="flex flex-col">
      {list.length > 0 ? (
        list.map((player, index) => {
          ...
          return (
            <div key={player._id || index} className="flex items-center justify-between py-4 border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors duration-150 cursor-pointer px-5" onClick={() => navigate('/team')}>
              <div className="flex items-center gap-5">
                {/* Rank Indicator */}
                <div className="w-8 flex items-center justify-center shrink-0">
                  {index === 0 ? (
                    <span className="text-2xl font-black text-[#00E5FF] drop-shadow-sm">1</span>
                  ) : (
                    <span className="text-xl font-bold text-slate-400">{index + 1}</span>
                  )}
                </div>

                {/* Player Info (NO pictures) */}
                <div className="flex flex-col min-w-0">
                  <h4 className="text-[16px] font-bold tracking-tight text-slate-800 truncate mb-[2px]">{player.firstName} {player.lastName}</h4>
                  <p className="text-[9px] text-slate-500 font-black tracking-[0.15em] uppercase truncate">{player.position || 'Unassigned'}</p>
                </div>
              </div>

              {/* Stat Value */}
              <div className="flex flex-col items-end shrink-0">
                <span className="text-[32px] font-['Barlow'] font-black text-slate-900 leading-[0.85] tracking-tight">{statValue}</span>
                <span className="text-[9px] font-bold text-cyan-600/80 uppercase tracking-widest mt-1.5">{label}</span>
              </div>
            </div>
          );
        })
        ...
        <div className="flex-1 flex flex-col items-center justify-center py-12 text-slate-400">
          <Activity size={24} className="mb-3 opacity-30 text-slate-300" />
          <span className="text-[10px] font-bold uppercase tracking-widest text-center text-slate-400">No match data<br />logged yet</span>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm flex flex-col font-['Inter']">
      {/* Card Header */}
      <div className="px-5 py-4 flex justify-between items-center bg-blue-900 border-b border-blue-900">
        <h3 className="font-bold tracking-tight text-white text-base">Player Leaderboard</h3>
      </div>

      {/* Subheader (Tabs Container) */}
      <div className="flex border-b border-slate-200 w-full bg-white">
        <button
          onClick={() => setActiveTab('goals')}
          className={`flex-1 py-3 px-6 bg-transparent rounded-none transition-colors border-b-[3px] ${activeTab === 'goals' ? 'border-[#00E5FF] text-slate-900 font-bold' : 'border-transparent text-slate-400 font-medium hover:text-slate-500'}`}
        >
          Goals
        </button>
        <button
          onClick={() => setActiveTab('assists')}
          className={`flex-1 py-3 px-6 bg-transparent rounded-none transition-colors border-b-[3px] ${activeTab === 'assists' ? 'border-[#00E5FF] text-slate-900 font-bold' : 'border-transparent text-slate-400 font-medium hover:text-slate-500'}`}
        >
          Assists
        </button>
        <button
          onClick={() => setActiveTab('contributions')}
          className={`flex-1 py-3 px-6 bg-transparent rounded-none transition-colors border-b-[3px] ${activeTab === 'contributions' ? 'border-[#00E5FF] text-slate-900 font-bold' : 'border-transparent text-slate-400 font-medium hover:text-slate-500'}`}
        >
          Contribs
        </button>
      </div>

      <div className="p-0 flex-1 flex flex-col min-h-[180px]">
      ...
      <div className="bg-slate-50 p-3 border-t border-slate-100 mt-auto flex justify-center">
        <Button size="sm" variant="ghost" className="text-[10px] font-bold text-white bg-blue-900 hover:bg-blue-800 transition-colors uppercase tracking-widest w-full rounded-md" onClick={() => navigate('/team')}>
          Full Squad Output
        </Button>
      </div>
    </div>
...
const _OldPlayerLeaderboard = ({ players, navigate }) => {
  ...
  return (
    ...
    <div className="font-sans tracking-tight text-slate-800">
        ...
        <div className="max-w-[1600px] mx-auto">
          ...
          <div className="flex items-center justify-between px-6 lg:px-10 py-3 border-b border-transparent">
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigate('/')}>
              <img src="/Logo.png" alt="TactAIQ Logo" className="w-16 h-16 object-contain" />
              <div className="flex flex-col">
                <span className="text-xl leading-none tracking-tight drop-shadow-sm">
                  <span className="text-white font-bold">TACT</span>
                  <span className="text-[#06b6d4] font-bold">AIQ</span>
                </span>
                <span className="text-sm font-medium text-white/70 mt-0.5 tracking-wide">Master The Plan</span>
              </div>
            </div>
            ...
            <div className="hidden md:flex items-center gap-6 text-sm ml-8 lg:ml-12 flex-1">
              ...
              <button className="bg-transparent text-white/80 hover:text-white transition-colors font-medium pb-1" onClick={() => navigate('/team')}>Team Roaster</button>
              <button className="bg-transparent text-white/80 hover:text-white transition-colors font-medium pb-1" onClick={() => navigate('/events')}>Events</button>
              <button className="bg-transparent text-white/80 hover:text-white transition-colors font-medium pb-1" onClick={() => navigate('/library')}>Library</button>
              <button className="bg-transparent text-white/80 hover:text-white transition-colors font-medium pb-1" onClick={() => navigate('/video-analyser')}>Video Analytics</button>
            </div>
            ...
            <div className="flex items-center gap-5">
              ...
              <div className="relative profile-dropdown ml-2">
                <div className="flex items-center cursor-pointer bg-[#1e293b] rounded-[18px] p-1 pr-3 border border-white/5 hover:bg-[#334155] transition-colors shadow-sm group" onClick={() => setIsDropdownOpen(!isDropdownOpen)}>
                  {user?.profilePicture ? (
                  ...
                  <div className="flex flex-col hidden sm:flex justify-center ml-3 mr-2">
                    ...
                    <span className="text-[9px] font-black tracking-widest text-[#3b82f6] uppercase leading-none">{user?.role === 'coach' ? 'COACH' : (user?.role || 'TACTICIAN')}</span>
                  </div>
                  <ChevronDown size={14} strokeWidth={2.5} className={`text-white/40 ml-1 hidden sm:block transition-transform duration-300 ${isDropdownOpen ? 'rotate-180' : ''}`} />
                </div>
                ...
                {isDropdownOpen && (
                  <div className="absolute right-0 top-full mt-3 w-56 bg-slate-900 border border-slate-700 rounded-lg shadow-lg py-1 z-50 animate-in fade-in zoom-in-95 duration-200 origin-top-right">
                    <div className="px-4 py-2 flex flex-col items-start border-b border-slate-800 mb-1">
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Coach</p>
                    ...
                    <button
                      onClick={() => { setIsDropdownOpen(false); navigate('/team-setup'); }}
                      className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-slate-200 bg-transparent hover:bg-slate-800 border-l-2 border-transparent hover:border-cyan-400 transition-colors cursor-pointer"
                    >
          ...
          <div className="px-6 lg:px-10 py-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex items-center gap-4 mt-4 md:mt-0">
              <button
                className="flex items-center gap-2 bg-slate-800 text-white hover:bg-slate-700 border border-slate-700 transition-colors font-bold px-5 py-2.5 rounded-xl shadow-sm tracking-wide text-sm"
      ...
      <PostMatchModal
        ...
        onSuccess={() => setShowPostMatchModal(false)}
        events={upcomingEvents.concat(recentMatches)} // Provides unlogged matches dropdown items
        players={players}
        token={token}
      />
    </div>
